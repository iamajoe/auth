...
func (a *API) loadFlowState(w http.ResponseWriter, r *http.Request) (context.Context, error) {
	...
	if err != nil {
		...
		if uerr != nil {
			...
			).WithInternalError(uerr)
		}

		q := getErrorQueryString(
			err,
			utilities.GetRequestID(ctx),
...
func (a *API) oAuthCallback(
	ctx context.Context,
	r *http.Request,
	providerType string,
) (*OAuthProviderData, error) {
	var rq url.Values
	...
	if err != nil {
		return nil, badRequestError(
			ErrorCodeOAuthProviderNotSupported,
	...
	if err != nil {
		return nil, internalServerError(
			"Unable to exchange external code: %s",
			oauthCode,
		).WithInternalError(err)
	}
